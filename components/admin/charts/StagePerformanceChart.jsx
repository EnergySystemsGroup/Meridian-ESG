...
/**
 * Bar chart component for displaying stage-by-stage performance metrics
 */
export function StagePerformanceChart({ 
  ...
  if (!dataKey || typeof dataKey !== 'string') {
    console.warn('StagePerformanceChart: dataKey must be a non-empty string');
    return (
      <div className="w-full flex items-center justify-center" style={{ height }}>
    ...
    if (active && payload && payload.length > 0 && payload[0]) {
      ...
      return (
        <div className="rounded-lg shadow-lg p-3" style={{ 
          ...
          <p className="text-sm capitalize" style={{ color: chartColors.text.secondary }}>
    ...
    <ChartErrorBoundary height={height} title={title}>
      <div className="w-full">
        ...
        <BarChart 
          ...
          role="img"
          aria-label={`Bar chart${title ? ` showing ${title}` : ''}${yAxisLabel ? ` with ${yAxisLabel} values` : ''}`}
        >
...
StagePerformanceChart.propTypes = {
