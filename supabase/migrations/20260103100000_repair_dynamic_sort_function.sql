-- REPAIR MIGRATION: Recreate get_funding_opportunities_dynamic_sort function
--
-- This migration repairs a missing function on staging caused by migration ordering issues.
-- The function was dropped by CASCADE in 20251213000002 but not properly recreated.
--
-- Safe to run on:
-- - Local: replaces existing function (no-op)
-- - Staging: creates missing function (repair)
-- - Production: no-op since original migration will create it first
--
...
BEGIN
    ...
    RAISE NOTICE 'Executing query (V16 - repair migration): %', final_query;
    RETURN QUERY EXECUTE final_query;

END;
$function$;

COMMENT ON FUNCTION get_funding_opportunities_dynamic_sort IS 'V16: Multi-status filter support (p_status as text[]). Repair migration 20260103100000.';
