-- Migration: Add program_id to funding_opportunities_with_geography view
-- Purpose: program_id was added to funding_opportunities (Phase 3) but never exposed
--          in the view. The admin review API needs it for program linking.
-- Created: 2026-02-18

-- CREATE OR REPLACE VIEW requires new columns appended at end.
-- This rebuilds the full view definition with program_id added after review_notes.
...
SELECT
    ...
    -- Columns added in 20260218000001
    fo.promotion_status,
    fo.reviewed_by,
    fo.reviewed_at,
    fo.review_notes,
    -- New: program_id (added in Phase 3, needed by admin review)
    fo.program_id
