-- Add total_funding_available column to funding_opportunities if it doesn't exist
ALTER TABLE funding_opportunities 
ADD COLUMN IF NOT EXISTS total_funding_available NUMERIC;

-- If the table already has a column named total_funding, copy values to total_funding_available
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 
        ...
        SET total_funding_available = total_funding
        WHERE total_funding_available IS NULL AND total_funding IS NOT NULL;
    END IF;
    
    -- If the table has a column named amount_available, fall back to that
    IF EXISTS (
        SELECT 1 
        FROM information_schema.columns 
        WHERE table_name = 'funding_opportunities' 
        ...
        UPDATE funding_opportunities 
        SET total_funding_available = amount_available
        WHERE total_funding_available IS NULL AND amount_available IS NOT NULL;
    END IF;
END $$;

-- Create an improved function that uses total_funding_available
