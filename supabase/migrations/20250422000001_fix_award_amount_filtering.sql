-- Update all functions to use only maximum_award for filtering by amount

-- Fix the total opportunities count function to only use maximum_award
...
BEGIN
    ...
    WHERE
        ...
        (p_min_amount IS NULL OR 
            fo.maximum_award >= p_min_amount OR
            (fo.maximum_award IS NULL AND fo.minimum_award >= p_min_amount)
        ) AND
        (p_max_amount IS NULL OR 
            fo.maximum_award <= p_max_amount OR
            (fo.maximum_award IS NULL AND fo.minimum_award <= p_max_amount)
        ) AND
...
-- Fix the total funding available function to only use maximum_award for filtering
...
-- Fix the states with funding count function to only use maximum_award for filtering
...
-- Update the per-applicant function to use only maximum_award for filtering
