-- Add p_tracked_ids parameter to get_funding_opportunities_dynamic_sort function
...
BEGIN
    ...
    -- Add filter for tracked IDs
    IF p_tracked_ids IS NOT NULL AND array_length(p_tracked_ids, 1) > 0 THEN
        where_clauses := array_append(where_clauses, format('id = ANY(%L::text[])', p_tracked_ids));
    END IF;
    ...
    RAISE NOTICE 'Executing query (V6): %', final_query;
