-- Fix function parameter names to match API expectations
-- and consolidate to a single, clean implementation
...
DROP FUNCTION IF EXISTS get_funding_by_state;

-- Create a single, clean implementation with parameter names matching the API
CREATE OR REPLACE FUNCTION get_funding_by_state(
    p_status TEXT DEFAULT NULL,
    p_source_type TEXT DEFAULT NULL, 
    p_min_amount NUMERIC DEFAULT NULL, -- Match API parameter name
    p_max_amount NUMERIC DEFAULT NULL, -- Match API parameter name
...
-- Add debug function to check California data specifically
...
BEGIN
    RETURN QUERY
    SELECT * FROM get_funding_by_state() WHERE state = 'California';
END;
$$ LANGUAGE plpgsql;
