-- Fix variable name conflicts in the get_funding_by_state function

-- Drop the existing function
DROP FUNCTION IF EXISTS get_funding_by_state(TEXT, TEXT, NUMERIC, NUMERIC, TEXT[]);

-- Create the updated function with qualified variable names
...
DECLARE
    -- Use explicit variable declarations to avoid conflicts
    v_state ALIAS FOR state;
    v_state_code ALIAS FOR state_code;
    v_value ALIAS FOR value;
    v_opportunities ALIAS FOR opportunities;
BEGIN
    RETURN QUERY
...
CREATE OR REPLACE FUNCTION get_california_funding() 
RETURNS TABLE (
    state TEXT,
    state_code TEXT,
    value NUMERIC,
...
BEGIN
    RETURN QUERY
    SELECT * FROM get_funding_by_state() 
    WHERE state_code = 'CA';
END;
$$ LANGUAGE plpgsql;
