-- Migration: Fix tracked IDs handling for empty arrays
-- When p_tracked_ids is an empty array, we should return no results (not all results)
...
CREATE OR REPLACE FUNCTION get_funding_opportunities_dynamic_sort(
    ...
    p_tracked_ids TEXT[] DEFAULT NULL
)
...
BEGIN
    ...
    -- FIXED: Handle tracked IDs filter properly
    -- If p_tracked_ids is provided (not NULL), we need to filter by those IDs
    -- If the array is empty, we should return no results
    IF p_tracked_ids IS NOT NULL THEN
        IF array_length(p_tracked_ids, 1) > 0 THEN
            where_clauses := array_append(where_clauses, format('id::text = ANY(%L::text[])', p_tracked_ids));
        ELSE
            -- Empty array means "no results" - add impossible condition
    ...
    RAISE NOTICE 'Executing query (V10 - fixed empty tracked IDs handling): %', final_query;
...
COMMENT ON FUNCTION get_funding_opportunities_dynamic_sort(TEXT, TEXT[], TEXT[], TEXT, TEXT, TEXT, INT, INT, TEXT[]) IS 'Get funding opportunities with dynamic sorting and filtering - fixed empty tracked IDs handling';