-- Migration: Add project types filtering to replace categories filter
-- This adds p_project_types parameter to the RPC function for filtering by eligible_project_types

-- First, drop the existing function (all signatures)
DROP FUNCTION IF EXISTS get_funding_opportunities_dynamic_sort(text, text[], text, text[], text, text, text, integer, integer, text[]);

-- Create the updated function with project types parameter
CREATE OR REPLACE FUNCTION public.get_funding_opportunities_dynamic_sort(
    p_status text DEFAULT NULL::text,
    p_categories text[] DEFAULT NULL::text[],
    p_project_types text[] DEFAULT NULL::text[],  -- NEW: array of project types to filter by
...
BEGIN
    ...
    RAISE NOTICE 'Executing query (V14 - with project types filtering): %', final_query;
    RETURN QUERY EXECUTE final_query;

END;
$function$;

COMMENT ON FUNCTION get_funding_opportunities_dynamic_sort IS 'V14: Added project types filtering parameter';
