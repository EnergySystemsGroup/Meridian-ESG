-- Add the definition for the second overload of get_funding_by_state
-- This is the one called by the API route, fixing the type mismatch.
...
BEGIN
    ...
    SELECT
        ...
        sa.state_code::text AS state_code, -- **** THE FIX IS HERE ****
        -- Add the state's direct value + a share of the national value (simple distribution for now)
        COALESCE(sa.total_value, 0) + COALESCE((SELECT national_total_value FROM national_aggregation) / NULLIF((SELECT COUNT(*) FROM states), 0), 0) AS value,
        -- Add the state's direct count + a share of the national count
        COALESCE(sa.opp_count, 0)::INTEGER + COALESCE((SELECT national_opp_count FROM national_aggregation) / NULLIF((SELECT COUNT(*) FROM states), 0), 0)::INTEGER AS opportunities
    FROM
