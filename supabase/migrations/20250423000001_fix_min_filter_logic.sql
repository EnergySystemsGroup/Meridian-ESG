-- Fix filtering logic to show only opportunities with maximum award at or above the filter amount
...
BEGIN
    ...
    WHERE
        ...
        -- Filter by minimum amount - show only opportunities with max award >= filter amount
        (p_min_amount IS NULL OR p_min_amount = 0 OR 
            COALESCE(fo.maximum_award, 0) >= p_min_amount
        ) AND
...
-- Fix the total funding available function
...
-- Fix the states with funding count function
...
-- Update the per-applicant function with the correct filtering logic
