...
DROP FUNCTION IF EXISTS get_total_funding_available(TEXT, TEXT, NUMERIC, NUMERIC, TEXT[], CHARACTER(2)) CASCADE;
DROP FUNCTION IF EXISTS get_total_funding_available(TEXT, TEXT, NUMERIC, NUMERIC, TEXT[]) CASCADE;
DROP FUNCTION IF EXISTS get_states_with_funding_count(TEXT, TEXT, NUMERIC, NUMERIC, TEXT[]) CASCADE;
DROP FUNCTION IF EXISTS get_total_opportunities_count(TEXT, TEXT, NUMERIC, NUMERIC, TEXT[]) CASCADE;

-- Now recreate the functions with the improved implementations
...
DECLARE
    opp_count INTEGER := 0;
BEGIN
    RAISE NOTICE 'Running get_total_opportunities_count with params: status=%, source_type=%, min=%, max=%, categories=%', 
    ...
    RAISE NOTICE 'Total opportunities found: %', COALESCE(opp_count, 0);
...
BEGIN
    RAISE NOTICE 'Running get_total_funding_available with params: status=%, source_type=%, min=%, max=%, categories=%, state=%', 
    ...
    IF p_state_code IS NOT NULL THEN
        ...
        RAISE NOTICE 'State code % resolved to ID %', p_state_code, state_id;
    END IF;
    ...
    RAISE NOTICE 'Total funding calculated: %', COALESCE(total, 0);
...
DECLARE
    ...
    opp_count INTEGER := 0;
BEGIN
    RAISE NOTICE 'Running get_states_with_funding_count with params: status=%, source_type=%, min=%, max=%, categories=%', 
        p_status, p_source_type, p_min_amount, p_max_amount, p_categories;
    ...
    RAISE NOTICE 'Total opportunities found for states count: %', opp_count;
    
    -- If no opportunities match the filters, return 0
    IF opp_count = 0 THEN
        RAISE NOTICE 'No opportunities found, returning 0 states';
        RETURN 0;
    END IF;
