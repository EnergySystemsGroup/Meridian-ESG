-- Make functions case-insensitive for status filtering

-- Fix the total opportunities count function to handle status case-insensitively
...
-- Fix the total funding available function to handle status case-insensitively
...
-- Fix the states with funding count function to handle status case-insensitively
...
-- Update the API endpoint function to be consistent
...
BEGIN
    RETURN QUERY SELECT * FROM get_funding_by_state_per_applicant(
        p_status,
        p_source_type,
        p_min_amount,
        p_max_amount,
...
-- Update the per-applicant function for case insensitivity
