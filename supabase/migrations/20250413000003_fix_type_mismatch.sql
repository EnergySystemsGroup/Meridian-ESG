-- Fix type mismatch issues with the get_funding_by_state function

-- Drop the existing function
DROP FUNCTION IF EXISTS get_funding_by_state(TEXT, TEXT, NUMERIC, NUMERIC, TEXT[]);
DROP FUNCTION IF EXISTS get_california_funding();

-- Final clean function with explicit type casting
...
BEGIN
    ...
    WITH filtered_opportunities AS (
        ...
        SELECT fo.id, fo.maximum_award, fo.minimum_award, fo.is_national
    ...
    state_specific_data AS (
        -- Get state-specific opportunities with explicit casting
        SELECT 
            s.name::TEXT AS state_name,
    ...
    all_states AS (
        ...
        SELECT name::TEXT, code::TEXT FROM states
    ),
    combined_data AS (
        -- Combine state-specific and national data
        SELECT
            als.name::TEXT AS state_name,
            als.code::TEXT AS state_code,
            COALESCE(ssd.state_funding, 0)::NUMERIC AS state_funding,
            COALESCE(ssd.state_opp_count, 0)::INTEGER AS state_opp_count,
            COALESCE((SELECT national_funding FROM national_data), 0)::NUMERIC AS national_funding,
            COALESCE((SELECT national_opp_count FROM national_data), 0)::INTEGER AS national_opp_count
        FROM
            all_states als
        LEFT JOIN
            state_specific_data ssd ON als.code = ssd.state_code
    )
    -- Output final result with explicit names and types
    SELECT
        cd.state_name::TEXT AS state,
        cd.state_code::TEXT AS state_code,
        (cd.state_funding + cd.national_funding)::NUMERIC AS value,
        (cd.state_opp_count + cd.national_opp_count)::INTEGER AS opportunities
    FROM
        combined_data cd
    ORDER BY
        cd.state_name;
