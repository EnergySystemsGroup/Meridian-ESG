-- Add category filtering support to the funding calculation function

-- Drop the existing function to replace it with the enhanced version
DROP FUNCTION IF EXISTS get_funding_by_state(TEXT, TEXT, NUMERIC, NUMERIC);

-- Create the enhanced version with p_categories parameter
CREATE OR REPLACE FUNCTION get_funding_by_state(
    p_status TEXT DEFAULT NULL,
    p_source_type TEXT DEFAULT NULL, 
    p_min_award_val NUMERIC DEFAULT NULL,
    p_max_award_val NUMERIC DEFAULT NULL,
    p_categories TEXT[] DEFAULT NULL -- New parameter for category filtering
)
RETURNS TABLE (
    state TEXT,
    state_code TEXT,
...
BEGIN
    ...
    WITH filtered_opportunities AS (
        ...
        WHERE
            ...
            (p_min_award_val IS NULL OR 
                COALESCE(fo.minimum_award, fo.maximum_award, 0) >= p_min_award_val
            ) AND
            (p_max_award_val IS NULL OR 
                COALESCE(fo.maximum_award, fo.minimum_award, 0) <= p_max_award_val
            ) AND
    ...
    SELECT
        state_name AS state,
        state_code,
        total_funding AS value,
        total_opp_count::INTEGER AS opportunities
    FROM
        combined_data
    ORDER BY
        state_name;
