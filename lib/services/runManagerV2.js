...
export class RunManagerV2 {
  ...
  async updateRunError(error, failedStage = null) {
    ...
    try {
      ...
      if (currentRun?.status === 'failed' || currentRun?.status === 'completed') {
        console.warn(`[RunManagerV2] ⚠️ Run ${this.runId} already in terminal state: ${currentRun.status}`);
        return;
      }
  ...
  async recordStageFailure(stageName, failureInfo) {
    ...
    try {
      ...
      const errorHistory = currentRun?.error_details?.history || [];
      errorHistory.push({
        stage: stageName,
        ...failureInfo,
        timestamp: new Date().toISOString()
