...
export class RunManagerV2 {
  ...
  async updateV2Analysis(status, analysisResult = null, metrics = null, tokensUsed = 0, apiCalls = 0, inputCount = 0, outputCount = 0, jobId = null) {
    // V2 clean metrics only
    await this.updateV2Stage('analysis', status, analysisResult, metrics, tokensUsed, apiCalls, inputCount, outputCount, jobId);
  }
  
  async updateV2Filter(status, filterResult = null, metrics = null, inputCount = 0, outputCount = 0, jobId = null) {
    // V2 clean metrics only
    await this.updateV2Stage('filter', status, filterResult, metrics, 0, 0, inputCount, outputCount, jobId);
  }
  
  async updateV2Storage(status, storageResult = null, metrics = null, tokensUsed = 0, inputCount = 0, outputCount = 0, jobId = null) {
    // V2 clean metrics only
    await this.updateV2Stage('storage', status, storageResult, metrics, tokensUsed, 0, inputCount, outputCount, jobId);
  }
  ...
  async updateV2Stage(stageName, status, stageResults = {}, performanceMetrics = {}, tokensUsed = 0, apiCalls = 0, inputCount = 0, outputCount = 0, jobId = null) {
    ...
    try {
      ...
      if (jobId) {
        // When jobId is provided, first look for existing stage for this specific job
        const { data: jobSpecificStage } = await this.supabase
        ...
        if (jobSpecificStage) {
          existingStage = jobSpecificStage;
        } else {
          // If no job-specific stage found, look for stage with NULL job_id that we can update
          const { data: nullJobStage } = await this.supabase
            .from('pipeline_stages')
            .select('id, started_at, completed_at')
            .eq('run_id', this.v2RunId)
            .eq('stage_name', stageName)
      ...
      if (existingStage) {
        ...
        // Set job_id if we're updating a NULL job_id record with a specific jobId
        if (jobId) {
        ...
        const { error } = await this.supabase
          ...
          .insert({
            ...
            execution_time_ms: status === 'completed' && executionTime !== null && executionTime !== undefined ? this.sanitizeInteger(executionTime) : null,
            api_calls_made: this.sanitizeInteger(apiCalls),
            tokens_used: this.sanitizeInteger(tokensUsed),
            estimated_cost_usd: parseFloat(estimatedCost.toFixed(6)),
            input_count: this.sanitizeInteger(inputCount),
