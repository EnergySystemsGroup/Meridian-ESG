...
export class RunManagerV2 {
  ...
  async updateV2EarlyDuplicateDetector(status, detectionResult = null, metrics = null, inputCount = 0, outputCount = 0) {
    ...
    const performanceMetrics = metrics ? { executionTime: metrics.executionTime || 0 } : {};
    await this.updateV2Stage('early_duplicate_detector', status, detectionResult, performanceMetrics, 0, 0, inputCount, outputCount);
  ...
  async completeRun(totalTime = null, finalResults = null) {
    ...
    if (completionRunData && completionRunData.total_opportunities_processed > 0) {
      const totalOpportunities = completionRunData.total_opportunities_processed;
      const totalTokens = completionRunData.total_tokens_used || 0;
      const totalCost = completionRunData.estimated_cost_usd || 0;
