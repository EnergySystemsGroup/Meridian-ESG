...
export async function processApiSourceV2(sourceId, runId = null, supabase, anthropic, options = {}) {
  ...
  try {
    ...
    try {
      ...
      // ⚡ DEBUG: Log extraction result before database update
      console.log(`[ProcessCoordinatorV2] 🔍 DEBUG - Data Extraction Result:`, {
        extractionResultType: typeof extractionResult,
        opportunitiesLength: extractionResult.opportunities?.length,
        extractionMetricsKeys: Object.keys(extractionResult.extractionMetrics || {}),
        ...
        firstOpportunityKeys: extractionResult.opportunities?.[0] ? Object.keys(extractionResult.opportunities[0]) : 'none'
      });
      
      // Data Extraction: input is source orchestrator output (1), output is number of opportunities extracted
      await runManager.updateV2DataExtraction('completed', extractionResult, extractionResult.extractionMetrics, extractionTokens, extractionApiCalls, 1, extractionResult.opportunities.length)
