...
export async function processApiSourceV2(sourceId, runId = null, supabase, anthropic, options = {}) {
  ...
  try {
    ...
    // Calculate token savings percentage (estimated)
    if (metrics.optimizationImpact.totalOpportunities > 0) {
      metrics.optimizationImpact.tokenSavingsPercentage = Math.round(
        (bypassedOpportunities / metrics.optimizationImpact.totalOpportunities) * 100
      )
    }
    ...
    console.log(`[ProcessCoordinatorV2] ✅ Duplicate detection completed: ${duplicateDetection.metrics.newOpportunities} new, ${duplicateDetection.metrics.opportunitiesToUpdate} to update, ${duplicateDetection.metrics.opportunitiesToSkip} to skip (${metrics.optimizationImpact.tokenSavingsPercentage}% optimization)`)
    ...
    // Calculate time savings percentage (estimated)
    if (metrics.optimizationImpact.bypassedLLM > 0) {
      // Estimate time saved by bypassing LLM processing (avg 3-5 seconds per opportunity)
      const estimatedTimeSaved = metrics.optimizationImpact.bypassedLLM * 4000 // 4 seconds in ms
      const estimatedOriginalTime = executionTime + estimatedTimeSaved
      metrics.optimizationImpact.timeSavingsPercentage = Math.round((estimatedTimeSaved / estimatedOriginalTime) * 100)
    }
    
    // Calculate efficiency score (0-100)
    const efficiencyFactors = [
      metrics.optimizationImpact.tokenSavingsPercentage,
      metrics.optimizationImpact.timeSavingsPercentage,
      (metrics.stageMetrics.storage?.successRate || 100),
      (metrics.stageMetrics.directUpdate?.successRate || 100)
    ].filter(factor => factor > 0)
    
    const efficiencyScore = efficiencyFactors.length > 0 
      ? Math.round(efficiencyFactors.reduce((sum, factor) => sum + factor, 0) / efficiencyFactors.length)
      : 0
    
    // Update final optimization metrics in RunManager
    await runManager.updateOptimizationMetrics({
      ...
      estimatedCost: metrics.totalTokensUsed * 0.00001 // Rough estimate
    })
    ...
    console.log(`[ProcessCoordinatorV2] 📊 Optimization Impact: ${metrics.optimizationImpact.tokenSavingsPercentage}% token savings, ${metrics.optimizationImpact.timeSavingsPercentage}% time savings, ${efficiencyScore}% efficiency score`)
