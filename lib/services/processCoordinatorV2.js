...
export async function processApiSourceV2(sourceId, runId = null, supabase, anthropic, options = {}) {
  ...
  try {
    ...
    let sourceAnalysis;
    try {
      sourceAnalysis = await analyzeSource(source, anthropic)
      ...
      console.error(`[ProcessCoordinatorV2] ❌ SourceOrchestrator failed:`, error)
      await runManager.updateV2SourceOrchestrator('failed', null, null, 0, 0, 0, 0)
      throw new Error(`SourceOrchestrator stage failed: ${error.message}`)
    }
    ...
    try {
      extractionResult = await extractFromSource(source, sourceAnalysis, anthropic)
      ...
      const extractedCount = extractionResult.opportunities.length
      await runManager.updateV2DataExtraction('completed', extractionResult, extractionResult.extractionMetrics, extractionTokens, extractionApiCalls, 1, extractedCount)
      console.log(`[ProcessCoordinatorV2] ✅ DataExtraction completed: ${extractedCount} opportunities extracted (${extractionTokens} tokens)`)
    } catch (error) {
      console.error(`[ProcessCoordinatorV2] ❌ DataExtractionAgent failed:`, error)
      await runManager.updateV2DataExtraction('failed', null, null, 0, 0, 1, 0)
      throw new Error(`DataExtractionAgent stage failed: ${error.message}`)
    }
    ...
    if (duplicateDetection.newOpportunities.length > 0) {
      ...
      try {
        ...
        if (analysisInputCount !== dupeOutputCount) {
          console.warn(`[ProcessCoordinatorV2] ⚠️ Stage handoff mismatch [early_duplicate_detector → analysis]: expected ${dupeOutputCount} opportunities, got ${analysisInputCount}`);
        }
