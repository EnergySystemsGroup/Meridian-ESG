...
export async function processJob(jobData, supabase = null, anthropic = null, runManager = null) {
  ...
  try {
    ...
    await runManager.updateV2DataExtraction('in_progress', null, {}, 0, 0, jobData.chunkedData.length, 0);
    ...
    }, extractionResult.extractionMetrics?.totalTokens || 0, extractionResult.extractionMetrics?.totalApiCalls || 0, jobData.chunkedData.length, trackedOpportunities.length);
    ...
    if (duplicateResult.newOpportunities && duplicateResult.newOpportunities.length > 0) {
      ...
      }, analysisResult.analysisMetrics?.totalTokens || 0, analysisResult.analysisMetrics?.totalApiCalls || 0, duplicateResult.newOpportunities.length, enhancedOpportunities.length);
    ...
    if (filteredOpportunities.length > 0) {
      ...
      }, storageResults.metrics?.totalTokens || 0, filteredOpportunities.length, storageResults.results?.filter(r => r.success).length || 0);
