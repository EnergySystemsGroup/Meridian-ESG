...
export async function processJob(jobData, supabase = null, anthropic = null, runManager = null) {
  ...
  try {
    ...
    // Check if force full processing is enabled (from job config OR database flag)
    const forceFullProcessing = jobData.forceFullProcessing || await shouldForceFullProcessing(jobData.sourceId, supabase);
    if (forceFullProcessing) {
      console.log(`[JobProcessor] ⚠️ Force full reprocessing enabled - will bypass duplicate detection (source: ${jobData.forceFullProcessing ? 'job config' : 'database flag'})`);
    }
    ...
    console.log(`[JobProcessor] 💾 Storage stage check:`, {
      filteredCount: filteredOpportunities.length,
      enhancedCount: enhancedOpportunities.length,
      willRunStorage: filteredOpportunities.length > 0,
      filteredOpportunitiesPreview: filteredOpportunities.map(o => ({
        id: o.id,
        title: o.title?.substring(0, 50) + '...',
        finalScore: o.scoring?.finalScore
      }))
    });
    ...
    if (filteredOpportunities.length > 0) {
      ...
      const opportunitiesStored = storageResults.results?.newOpportunities?.length || 0;
      const successRate = filteredOpportunities.length > 0 ? (opportunitiesStored / filteredOpportunities.length * 100).toFixed(1) : 0;
      ...
      await runManager.updateV2Storage('completed', storageResults, {
        ...
        opportunities_stored: opportunitiesStored,
        storage_errors: storageResults.metrics?.error ? 1 : 0
      }, storageResults.metrics?.totalTokens || 0, filteredOpportunities.length, opportunitiesStored, jobData.jobId);
      
      console.log(`[JobProcessor] ✅ Storage complete: ${opportunitiesStored}/${filteredOpportunities.length} opportunities stored (${successRate}% success rate)`);
    } else {
      console.log(`[JobProcessor] ⚠️ CRITICAL: No filtered opportunities for storage stage!`);

      if (enhancedOpportunities.length > 0) {
        console.warn(`[JobProcessor] 🚨 WARNING: ${enhancedOpportunities.length} opportunities were enhanced but ALL filtered out!`);
        console.warn(`[JobProcessor] 🔍 Sample enhanced opportunity that was filtered:`, {
          id: enhancedOpportunities[0].id,
          title: enhancedOpportunities[0].title?.substring(0, 100),
          scoring: enhancedOpportunities[0].scoring,
          hasScoring: !!enhancedOpportunities[0].scoring,
          finalScore: enhancedOpportunities[0].scoring?.finalScore
        });
      }
    }
    ...
    const results = {
      ...
      storage: {
        newStored: storageResults?.results?.newOpportunities?.length || 0,
        newErrors: storageResults?.metrics?.error ? 1 : 0,
        updated: updateResults?.successful || 0,
        updateErrors: updateResults?.failed || 0
      },
      
      // Overall metrics
      totalProcessed: (storageResults?.results?.newOpportunities?.length || 0) + (updateResults?.successful || 0),
      totalErrors: (storageResults?.metrics?.error ? 1 : 0) + (updateResults?.failed || 0),
