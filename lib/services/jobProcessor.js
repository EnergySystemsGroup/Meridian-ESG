...
import { createClient } from '../../utils/supabase/api.js';
import { createAnthropicClient } from '../agents-v2/utils/anthropicClient.js';
 ...
 */
export async function processJob(jobData, supabase = null, anthropic = null) {
  ...
  try {
    ...
    // Initialize RunManager for this job chunk
    // Link to master run if provided, otherwise create standalone run
    ...
    await runManager.updateV2DataExtraction('in_progress', null, {}, jobData.chunkedData.length, 0);
    ...
    await runManager.updateV2DataExtraction('success', extractionResult, {
    ...
    }, jobData.chunkedData.length, trackedOpportunities.length, extractionResult.extractionMetrics?.totalTokens || 0, extractionResult.extractionMetrics?.totalApiCalls || 0);
    ...
    await runManager.updateV2EarlyDuplicateDetector('in_progress', null, {}, trackedOpportunities.length, 0);
    ...
    await runManager.updateV2EarlyDuplicateDetector('success', duplicateResult, {
    ...
    }, trackedOpportunities.length, (duplicateResult.newOpportunities?.length || 0) + (duplicateResult.opportunitiesToUpdate?.length || 0));
    ...
    if (duplicateResult.newOpportunities && duplicateResult.newOpportunities.length > 0) {
      ...
      await runManager.updateV2Analysis('in_progress', null, {}, duplicateResult.newOpportunities.length, 0);
      ...
      await runManager.updateV2Analysis('success', analysisResult, {
      ...
      }, duplicateResult.newOpportunities.length, enhancedOpportunities.length, analysisResult.analysisMetrics?.totalTokens || 0, analysisResult.analysisMetrics?.totalApiCalls || 0);
    ...
    if (enhancedOpportunities.length > 0) {
      ...
      await runManager.updateV2Filter('in_progress', null, {}, enhancedOpportunities.length, 0);
      ...
      await runManager.updateV2Filter('success', filterResult, {
      ...
      }, enhancedOpportunities.length, filteredOpportunities.length);
    ...
    if (filteredOpportunities.length > 0) {
      ...
      await runManager.updateV2Storage('in_progress', null, {}, filteredOpportunities.length, 0);
      ...
      await runManager.updateV2Storage('success', storageResults, {
      ...
      }, filteredOpportunities.length, storageResults.results?.filter(r => r.success).length || 0, storageResults.metrics?.totalTokens || 0);
    ...
    if (duplicateResult.opportunitiesToUpdate && duplicateResult.opportunitiesToUpdate.length > 0) {
      ...
      await runManager.updateV2DirectUpdate('in_progress', null, {}, duplicateResult.opportunitiesToUpdate.length, 0);
      ...
      const updateSuccessRate = duplicateResult.opportunitiesToUpdate.length > 0 ? (updateResults.successful / duplicateResult.opportunitiesToUpdate.length * 100).toFixed(1) : 0;
      
      await runManager.updateV2DirectUpdate('success', updateResults, {
        execution_time_ms: updateTime,
        update_success_rate: parseFloat(updateSuccessRate),
        opportunities_updated: updateResults.successful || 0,
        update_errors: updateResults.failed || 0
      }, duplicateResult.opportunitiesToUpdate.length, updateResults.successful || 0);
      
      console.log(`[JobProcessor] ✅ Direct updates complete: ${updateResults.successful || 0}/${duplicateResult.opportunitiesToUpdate.length} opportunities updated (${updateSuccessRate}% success rate)`);
    } else {
    ...
    // Update run manager with error and cleanup
    if (runManager) {
      try {
