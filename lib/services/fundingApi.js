...
export const fundingApi = {
  ...
  getOpportunities: async (supabase, filters = {}) => {
    try {
      ...
      const params = {
        p_status: filters.status?.length > 0 ? filters.status : null,  // Array of statuses
        p_categories:
        ...
        p_project_types:
          filters.projectTypes && filters.projectTypes.length > 0
            ...
            : null,  // Array of project types for filtering
  ...
  // No promotion_status filter — detail page works for all records (admin accesses pending_review by ID)
  getOpportunityById: async (supabase, id) => {
    try {
      ...
      if (data.api_sources) {
        ...
        delete opportunityData.api_sources;
      } else {
        opportunityData.api_source_url = null;
      }
