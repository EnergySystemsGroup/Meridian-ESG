...
export const fundingApi = {
  ...
  getOpportunities: async (supabase, filters = {}) => {
    try {
      ...
      if (params.p_tracked_ids !== null) {
        if (params.p_tracked_ids.length > 0) {
          countQuery = countQuery.in('id', params.p_tracked_ids);
        } else {
          // Empty tracked IDs array means "no results"
          countQuery = countQuery.eq('id', 'impossible-id-that-does-not-exist');
        }
      }
      if (params.p_search) {
