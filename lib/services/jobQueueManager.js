...
export class JobQueueManager {
  ...
  async updateJobStatus(jobId, status, options = {}) {
    try {
      ...
      // Integrate with RunManagerV2 if available
      if (this.runManager) {
        try {
          const queueResult = {
            jobId,
            ...
            masterRunId: data.master_run_id,
            ...options
          };
          
          // Job queue operations are tracked in processing_jobs table
          // Pipeline stages are for actual processing stages only
        } catch (runManagerError) {
          // Don't fail the job update if RunManager integration fails
          console.warn(`[JobQueueManager] ⚠️ RunManager integration failed:`, runManagerError.message);
        }
      }
