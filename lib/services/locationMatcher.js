...
export async function linkOpportunityToCoverageAreas(opportunityId, locationTexts) {
  ...
  // Upsert into junction table (using high-confidence matches only)
  // Uses ignoreDuplicates to handle re-processing without errors
  const upsertPromises = matchResults.matches
    ...
    .map(match =>
      supabase
        ...
        .upsert({
          ...
          onConflict: 'opportunity_id,coverage_area_id',
