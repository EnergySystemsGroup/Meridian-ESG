...
export const TAXONOMIES = {
	 ...
	 */
	SELECTABLE_PARENT_TYPES: ['K-12 School Districts'],
};
...
/**
 * Get expanded client types for matching (child → parent expansion)
 *
 * When a client selects a child type, they should match opportunities
 * that list either the child type OR its parent type.
 *
 * @param {string} clientType - The client's selected type
 * @returns {string[]} Array of types to match against (self + parent if applicable)
 */
export function getExpandedClientTypes(clientType) {
	// Check if this type is a child of any parent
	for (const [parent, children] of Object.entries(
		TAXONOMIES.CLIENT_TYPE_HIERARCHY
	)) {
		if (children.includes(clientType)) {
			return [clientType, parent]; // Return self + parent
		}
	}
	// Not a child - return self only (standalone or parent type)
	return [clientType];
}
...
export function getSelectableClientTypes() {
	// Get all children from hierarchy
	const allChildren = Object.values(TAXONOMIES.CLIENT_TYPE_HIERARCHY).flat();

	// Combine with standalone types and selectable parent exceptions
	const selectableTypes = [
		...allChildren,
		...TAXONOMIES.STANDALONE_CLIENT_TYPES,
		...TAXONOMIES.SELECTABLE_PARENT_TYPES, // e.g., "K-12 School Districts"
	];

	return selectableTypes.sort();
}
