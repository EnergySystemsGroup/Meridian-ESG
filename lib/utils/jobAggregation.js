 ...
 */
export function extractJobList(stages) {
  // Get unique job IDs
  ...
  return uniqueJobIds.map(jobId => {
    ...
    return {
      jobId,
      chunkIndex: firstStage?.chunk_index || 0,
      totalChunks: firstStage?.total_chunks || 1,
      status: getJobStatus(jobStages),
      createdAt: Math.min(...jobStages.map(s => new Date(s.created_at).getTime())),
      completedAt: getJobCompletedAt(jobStages),
