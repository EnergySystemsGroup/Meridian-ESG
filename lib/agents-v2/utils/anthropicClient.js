...
export class AnthropicClient {
  constructor(config = {}) {
    ...
    this.defaultConfig = {
      model: "claude-haiku-4-5-20251001",  // Default: fast, cost-efficient. Analysis uses Sonnet via PIPELINE_MODELS override.
      maxTokens: 8192,  // Haiku 4.5's max output capacity
      temperature: 0,
      retries: 4,
...
export const schemas = {
      ...
      opportunities: {
        ...
        items: {
          ...
          properties: {
            ...
            description: {
              type: "string",
              description: "Extract and combine ALL text fields from the API response verbatim. Include every field containing descriptive text, application details, contact information, dates, deadlines, or program information. Preserve everything as-is."
            },
            fundingType: {
              type: "string",
              nullable: true,
              description: "Type of funding (grant, loan, incentive, tax_credit, etc.)"
            },
            incentiveStructure: {
              type: ["string", "null"],
              enum: ["prescriptive", "deemed_calculated", "custom_performance", "make_ready", "direct_install", "financing", "audit_assessment", null],
              description: "How funding flows to the applicant. prescriptive=published $/unit tables, deemed_calculated=formula $/kWh or $/therm, custom_performance=funder reviews proposal, make_ready=applicant hires contractor and gets reimbursed, direct_install=funder performs work, financing=OBF/PACE/loan, audit_assessment=audits only. null if not applicable (most grants)."
            },
            funding_source: {
              type: "object",
              properties: {
                ...
                type: {
                  type: "string",
                  description: "High-level type (Federal, State, Utility, Foundation, County, Municipality, Other)"
                },
                website: {
                  type: "string",
                  nullable: true,
            ...
            eligibleProjectTypes: {
              type: "array",
              items: { type: "string" },
              description: "Specific components/equipment/systems being funded (HVAC Systems, Solar Panels, etc.) - THE STAR for client matching"
            },
            eligibleLocations: {
              type: "array",
              items: { type: "string" },
              nullable: true,
              description: "Array of geographic locations. CRITICAL: Extract ONLY the most granular boundaries. NEVER include a state if ANY utility territories or counties within that state are mentioned - even if the state is also mentioned in the source text. Priority: utility territories > counties > states. If source says 'California... PG&E, SCE, SDG&E territories' → extract ONLY ['PG&E service territory', 'SCE service territory', 'SDG&E service territory'], EXCLUDE 'California'. REGIONAL DESCRIPTORS: If a sub-state region is mentioned (e.g., 'Bay Area', 'Central Valley', 'Southern California'), use your knowledge to expand it to the specific counties in that region. Example: 'Bay Area' → ['Alameda County', 'Contra Costa County', 'Marin County', 'Napa County', 'San Francisco County', 'San Mateo County', 'Santa Clara County', 'Solano County', 'Sonoma County']. Use isNational for nationwide."
            },
            url: {
              type: "string",
              nullable: true,
            ...
            categories: {
              type: "array",
              items: { type: "string" },
              description: "Broad funding domain (Energy, Infrastructure, Education, etc.) - high-level organization"
            },
            tags: {
              type: "array",
              items: { type: "string" },
            ...
            awardProcess: {
              type: "string",
              enum: ["competitive", "first-come-first-served", "lottery", "formula-based", "rolling", "unknown"],
              nullable: true,
              description: "Selection process for awards - must be one of the enum values"
            },
            eligibleActivities: {
              type: "array",
              items: { type: "string" },
              description: "Actions/uses of funding (Installation, Equipment Purchase, Research, etc.) - what can be done with funding"
            },
            api_updated_at: {
              type: "string",
              nullable: true,
  ...
  /**
   * @deprecated Opportunity Analysis Schema - REFERENCE ONLY
   *
   * This monolithic LLM schema is deprecated but KEPT AS DOCUMENTATION of the
   * TARGET MERGED STRUCTURE that parallelCoordinator.js produces.
   *
   * Current v2 architecture:
   * - contentEnhancement schema: LLM generates 6 content fields
   * - scoringAnalyzer.js: DETERMINISTIC scoring (no LLM, no schema)
   * - parallelCoordinator.js: Merges extraction + content + scoring
   *
   * The structure below shows what the FINAL merged opportunity object looks like:
   * - Lines with "EXTRACTED DATA" = from Data Extraction Agent
   * - Lines with "ANALYSIS ENHANCEMENTS" = from Analysis Agent (content + scoring)
   *
   * DO NOT USE AS LLM SCHEMA - Reference only for understanding merged structure.
   */
  opportunityAnalysis: {
    type: "object",
    properties: {
        ...
        items: {
          ...
          properties: {
            ...
            eligibleLocations: {
              ...
              items: { type: "string" },
              nullable: true,
              description: "Array of geographic locations. Use MOST SPECIFIC level. Priority: utility territories > counties > states. NEVER include state if utilities/counties within it are mentioned. REGIONAL DESCRIPTORS: Expand sub-state regions (Bay Area, Central Valley, Southern California) to their specific counties using your knowledge. Use isNational for nationwide."
            },
            url: {
              type: "string",
              nullable: true,
        ...
        items: {
          ...
          properties: {
            ...
            programOverview: {
              type: "string",
              nullable: false,
              description: "2-3 sentence elevator pitch stating what it funds, award amounts, who can apply, and unique strategic value. Keep under 75 words total. Focus on what makes this worth pursuing."
            },
            programUseCases: {
              type: "string",
              nullable: false,
              description: "3-4 specific use cases as bulleted list showing how clients could use this funding. Each bullet: client type, problem, how funding helps. Be specific—avoid generic statements."
            },
            applicationSummary: {
              type: "string",
              nullable: false,
              description: "Bulleted list with 4-5 items including: process steps, timeline, key scoring factor, important submissions clients must provide, and success tip. Focus on decision-relevant information."
            },
            programInsights: {
              type: "string",
              nullable: false,
              description: "2-3 bullet points of important details: restrictions, significant guidelines, historical patterns, technical assistance, documentation challenges. Only include genuinely useful insights."
            },
            scoreAdjustment: {
              type: "integer",
              description: "Adjustment to the deterministic score (-3 to +3). Use 0 if the deterministic score is already right. Apply business judgment as an energy services GC: adjust when money doesn't flow through our scope, generic labels hide non-construction work, funding is dispersed too thin, niche qualifiers narrow the real pool, or program status issues."
            },
            adjustmentReasoning: {
              type: "string",
              description: "3-5 sentence concise business judgment from ESCO/GC perspective. Include the final adjusted score. Focus on WHY the adjustment was made (or why 0 adjustment). Example: 'Scores 6.0 (det. 9.0, adj. -3). HVAC in residential housing = window units, not commercial chillers. $3M across 500 units = $6K each.'"
            }
          },
          required: ["id", "enhancedDescription", "actionableSummary", "programOverview", "programUseCases", "applicationSummary", "programInsights", "scoreAdjustment", "adjustmentReasoning"]
        }
      }
    },
    required: ["analyses"]
  ...
  /**
   * @deprecated Scoring Analysis Schema - NOT USED
   *
   * Scoring is now 100% DETERMINISTIC in scoringAnalyzer.js using taxonomy tier matching.
   * This LLM schema was designed but never implemented - scoring uses pure JS functions:
   * - calculateTierScore() for taxonomy matching (hot=3, strong=2, mild=1, weak=0)
   * - calculateFundingScore() for dollar-based scoring
   * - calculateActivityMultiplier() for activity-based multiplier
   *
   * The actual scoring output uses: clientRelevance, projectTypeRelevance, fundingAttractiveness,
   * fundingType, activityMultiplier, baseScore, finalScore (NOT overallScore)
   *
