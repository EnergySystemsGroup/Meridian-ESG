 ...
 * @param {Object} impl - Optional implementation injection for testing (defaults to real implementation)
 * @returns {Promise<Object>} - Update results and metrics
 */
export async function updateDuplicateOpportunities(updateBatch, supabase, impl = { updateSingleOpportunity }) {
  ...
  try {
    for (const update of updateBatch) {
      const result = await impl.updateSingleOpportunity(update, supabase);
    ...
    // Calculate total processed as sum of all outcomes
    const totalProcessed = results.successful.length + results.failed.length + results.skipped.length;
    
    // Validation: ensure processing integrity (all inputs should be categorized)
    if (totalProcessed !== updateBatch.length) {
      console.error(`[DirectUpdateHandler] ❌ Processing integrity violation: input=${updateBatch.length}, processed=${totalProcessed}`);
      throw new Error(`DirectUpdate failed to process all opportunities: ${updateBatch.length} in, ${totalProcessed} processed`);
    }
