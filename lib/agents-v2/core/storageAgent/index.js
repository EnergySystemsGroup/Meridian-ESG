...
// stateEligibilityProcessor deprecated - now using opportunity_coverage_areas exclusively
...
async function processBatch(batch, source, client, forceFullProcessing = false) {
  ...
  const processingPromises = batch.map(async (opportunity) => {
    try {
      ...
      // Step 3: Link to coverage areas based on eligible_locations (can be done async)
      // Note: stateEligibilityProcessor deprecated - using opportunity_coverage_areas exclusively
        ...
        const coverageAreasPromise = linkOpportunityToCoverageAreas(inserted.id, locationTexts)
          .then(result => {
            if (result.success) {
              ...
              console.error(`[StorageAgent] ⚠️ Coverage area linking failed for ${inserted.id}:`, result.errors);
            }
          })
          .catch(err => console.error(`[StorageAgent] ⚠️ Coverage area linking failed for ${inserted.id}:`, err));
