...
function sanitizeFields(opportunity) {
  ...
  if (opportunity.scoring) {
    // Handle both new and old scoring formats
    const scoreValue = opportunity.scoring.adjustedScore ?? opportunity.scoring.finalScore ?? opportunity.scoring.overallScore;
    sanitized.relevance_score = sanitizeRelevanceScore(scoreValue);
  ...
  // Handle new analysis fields
  if (opportunity.programOverview) {
    sanitized.program_overview = sanitizeDescription(opportunity.programOverview);
  }
  if (opportunity.programUseCases) {
    sanitized.program_use_cases = sanitizeDescription(opportunity.programUseCases);
  }
  if (opportunity.applicationSummary) {
    sanitized.application_summary = sanitizeDescription(opportunity.applicationSummary);
  }
  if (opportunity.programInsights) {
    sanitized.program_insights = sanitizeDescription(opportunity.programInsights);
  }
