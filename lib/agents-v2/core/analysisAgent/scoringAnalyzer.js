 ...
 * @returns {number} Multiplier (1.0=hot, 0.75=strong, 0.5=mild, 0.15=weak)
 */
function calculateActivityMultiplier(activities, taxonomy) {
  if (!activities || activities.length === 0) return 0.15; // default to weak
  ...
  return 0.15; // weak tier — non-construction activities (training, research, admin)
}
 ...
 * @param {string|string[]} fundingType - Funding type(s) from opportunity (can be string, array, or JSON array string)
...
function calculateFundingTypeScore(fundingType, taxonomy) {
  ...
  // Normalize to array - handle string, JSON array string, or actual array
  let types = [];
  if (typeof fundingType === 'string') {
    if (fundingType.startsWith('[')) {
      try {
        types = JSON.parse(fundingType);
      } catch {
        types = [fundingType];
      }
    } else {
  ...
  } else if (Array.isArray(fundingType)) {
    types = fundingType;
  }

  // Case-insensitive matching helper
  const normalize = (s) => s?.toLowerCase?.() || '';
  const hotNorm = taxonomy.hot.map(normalize);
  const strongNorm = taxonomy.strong.map(normalize);
  const mildNorm = taxonomy.mild.map(normalize);

  // Return best score from any matching type
  if (types.some(t => hotNorm.includes(normalize(t)) || strongNorm.includes(normalize(t)))) return 1;
  if (types.some(t => mildNorm.includes(normalize(t)))) return 0.5;
  ...
  try {
    ...
    const scoringResults = opportunities.map((opportunity, index) => {
      try {
        ...
        console.error(`[ScoringAnalyzer] Error scoring opportunity ${opportunity.id}:`, oppError.message);
    ...
    console.error(`[ScoringAnalyzer] Scoring analysis failed:`, error.message);
