...
Incentive Structure: ${opp.incentiveStructure || 'N/A'}${scoringBlock}`;
}).join('\n\n')}
...
INCENTIVE STRUCTURE SIGNAL: If the opportunity has an incentiveStructure value, factor it into your adjustment:
- prescriptive (published $/unit tables) → upward pressure (+1 to +2), sales team can quote confident numbers
- deemed_calculated (formula-based) → slight upward pressure (+0.5 to +1)
- custom_performance (apply and find out) → neutral to slight downward (-0.5 to 0)
- make_ready (customer hires contractor) → downward (-1 to -2), limited scope control
- direct_install (funder does work) → strong downward (-2 to -3), contractor cut out
- financing (OBF/PACE) → neutral, depends on terms
- audit_assessment (audits only) → downward (-1 to -2), no implementation work
