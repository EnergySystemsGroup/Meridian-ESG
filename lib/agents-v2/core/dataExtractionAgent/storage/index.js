...
// Enhanced in-memory cache with size and memory management
const hashCache = new Map();
const MAX_CACHE_SIZE = ExtractionConfig.HASH_CACHE_SIZE;
const cacheStats = {
  hits: 0,
  misses: 0,
  evictions: 0,
...
/**
 * Update hash cache with size limit and memory monitoring
 */
function updateHashCache(key, value) {
  // Check memory pressure before adding to cache
  const memoryUsage = process.memoryUsage();
  const memoryThresholdMB = ExtractionConfig.MEMORY_THRESHOLD_MB * 0.8; // Use 80% of threshold for cache
  const currentMemoryMB = memoryUsage.heapUsed / (1024 * 1024);
  
  if (currentMemoryMB > memoryThresholdMB) {
    // Under memory pressure, clear half the cache
    const entriesToRemove = Math.floor(hashCache.size / 2);
    const iterator = hashCache.keys();
    for (let i = 0; i < entriesToRemove; i++) {
      const keyToRemove = iterator.next().value;
      hashCache.delete(keyToRemove);
      cacheStats.evictions++;
    }
    console.log(`[Storage] Memory pressure detected (${currentMemoryMB.toFixed(0)}MB), evicted ${entriesToRemove} cache entries`);
  }
  ...
  cacheStats.totalSize += valueSize;
  
  hashCache.set(key, { value, size: valueSize });
}
 ...
 * Optimized to avoid processing entire response - uses selective field extraction
 */
function generateContentHash(rawResponse, source) {
  try {
    // Check cache first (using a simple key based on response size and source)
    const cacheKey = `${source?.id || 'unknown'}_${JSON.stringify(rawResponse).length}`;
    const cachedHash = getFromCache(cacheKey);
    if (cachedHash) {
      console.log(`[DataExtractionAgent] 💾 Using cached hash for ${cacheKey}`);
      return cachedHash;
    }
    
    // Performance optimization: For large responses, only hash key fields
    const responseSize = JSON.stringify(rawResponse).length;
    const LARGE_RESPONSE_THRESHOLD = ExtractionConfig.LARGE_RESPONSE_THRESHOLD;
    
    if (responseSize > LARGE_RESPONSE_THRESHOLD) {
      console.log(`[DataExtractionAgent] 📊 Large response (${Math.round(responseSize/1024)}KB) - using optimized hashing`);
      
      // For large responses, only extract and hash critical fields
      const criticalData = {
        title: extractTitle(rawResponse, source),
        deadline: extractDeadline(rawResponse, source),
        agency: extractAgency(rawResponse, source),
        // Only use first 100 chars of description for large responses
        descSnippet: extractDescription(rawResponse, source)?.substring(0, 100)
      };
      
      // Remove empty values and create hash
      const cleanData = Object.fromEntries(
        Object.entries(criticalData).filter(([, v]) => v && v.toString().trim())
      );
      ...
      // Cache the result
      updateHashCache(cacheKey, hash);
      return hash;
    }
    
    // For smaller responses, use the existing comprehensive approach
    let hash;
    if (typeof rawResponse === 'object') {
      ...
      hash = crypto
        .createHash('sha256')
        .update(String(rawResponse).substring(0, 10000)) // Limit string hashing
        .digest('hex');
    }
